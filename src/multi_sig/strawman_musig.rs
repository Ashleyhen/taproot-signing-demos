...
pub fn test(){
	...
	let z=SecretKey::from_slice(&a_z.secret_key.secret_bytes()).unwrap().add_tweak(&Scalar::from_be_bytes(b_z.secret_key.secret_bytes()).unwrap()).unwrap();
	let shared_random_k=PublicKey::from_secret_key(&secp, &z).x_only_public_key().0;
	...
	let alice_sig=alice_pub_k.schnorr_sig_x_only(&msg,&aggregate_pub_k.x_only_public_key().0,&Scalar::from_be_bytes(a_z.secret_key.secret_bytes()).unwrap(),&shared_random_k);
	let bob_sig=bob_pub_k.schnorr_sig_x_only(&msg,&aggregate_pub_k.x_only_public_key().0,&Scalar::from_be_bytes(b_z.secret_key.secret_bytes()).unwrap(),&shared_random_k);
	...
	let sig=aggregate_sig_2(&secp,&alice_sig,&bob_sig);
	let sig_2=aggregate_sig(&secp,&msg,&aggregate_pub_k,&alice_sig,&bob_sig);
...
impl KeySet{
	    pub fn schnorr_sig_x_only(&self, msg: &Scalar, p:&XOnlyPublicKey,z:&Scalar,r:&XOnlyPublicKey) -> Vec<u8> {
			...
			let pk=PublicKey::from_secret_key(&secp, &random_keyset.secret_key);
		...
		// dbg!(r.serialize().to_hex());
			let mut signature = pk.x_only_public_key().0.serialize().to_vec();
