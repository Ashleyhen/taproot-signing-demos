...
pub fn compute_even_secret(secret: SecretKey, their_pub_k:&PublicKey)->SecretKey{
        ...
        if secret.public_key(&secp()).combine(their_pub_k).unwrap().x_only_public_key().1.eq(&Parity::Even) {
            return secret;
        }
...
pub fn test() {
   ...
   let d = [
    ...
    let r = [
        a[0].secondary_secret,
        a[1].secondary_secret,
    ];
   ...
   let decrypted_secret=compute_even_secret(new_secret_k(), &shared_aux);
    ...
    let a_pre_sig=partial_sig(
            &r[0].add_tweak(&Scalar::from_be_bytes(decrypted_secret.secret_bytes()).unwrap()).unwrap(),
            &e.to_vec(),
            &d[0].secret_bytes(),
        );
    ...
    let sig = [
        keyset_as_aux_for_sig(r[0], a_pre_sig[32..].try_into().unwrap()) ,
        partial_sig(
            &r[1],
            &e.to_vec(),
            &d[1].secret_bytes(),
    ...
    Checks::single_sig_check(&r[0],&message,&SecretKey::from_slice(&d[0].secret_bytes()).unwrap().secret_bytes());
    Checks::single_sig_check(&r[1],&message,&SecretKey::from_slice(&d[1].secret_bytes()).unwrap().secret_bytes());

    
    let signature=Signature::from_slice(&aggregate_sign(&sig)).unwrap();

    let final_sig=
    Signature::from_slice(&extract_sig_without_t(&decrypted_secret, &signature[..].to_vec())).unwrap();

dbg!( decrypted_secret.secret_bytes().to_hex());
dbg!( reveal_tweak(&signature[..].to_vec(), &final_sig[..].to_vec()).secret_bytes().to_hex());
