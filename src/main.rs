...
fn model() {
    ...
    let pks = sk.iter().map(|s|PublicKey::from_secret_key(&secp, &s)).collect::<Vec<PublicKey>>();
    ...
    let last_half:Vec<XOnlyPublicKey> = sk.iter().map(|secret|
        secret.mul_tweak(&h_p_scalar)
        .unwrap()
        .add_tweak(&random_scalar)
        .unwrap()
    ).map(|signer_a|PublicKey::from_secret_key(&secp, &signer_a).x_only_public_key().0).collect();

    let last_half_verification:Vec<XOnlyPublicKey> = pks.iter().map(|p|
    ...
    let s=last_half[0].add_tweak(&secp, &Scalar::from_be_bytes(last_half[0].serialize()).unwrap()).unwrap().0;

    dbg!(s.serialize().to_hex());
// (z1 + z2)*G + hash(r||P||m)*(k1 + k2)*G
let public_k=PublicKey::from_secret_key(&secp, &sk[0].add_tweak(&Scalar::from_be_bytes(sk[0].secret_bytes()).unwrap()).unwrap());
let combine=public_k.mul_tweak(&secp, &h_p_scalar).unwrap().add_exp_tweak(&secp,&Scalar::from_be_bytes(pk_r.x_only_public_key().0.serialize()).unwrap()).unwrap();
